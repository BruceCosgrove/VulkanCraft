...
namespace eng
{
        ...
        : Image(info.RenderContext)
    {
        ...
        ImageUtils::TransitionImageLayout(commandBuffer, m_Image, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, info.MipmapLevels);
        ...
        if (info.MipmapLevels > 1)
        {
            ...
            ImageUtils::GenerateMipmaps(commandBuffer, m_Image, extent, 1, info.MipmapLevels);
        }
        else
        {
            // Transition the image layout to be read from shaders.
            ImageUtils::TransitionImageLayout(commandBuffer, m_Image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, 1, info.MipmapLevels);
        }
